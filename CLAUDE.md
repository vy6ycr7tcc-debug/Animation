...
- 2026-09-26: Samuel: "for flying… it becomes a flame instead of the body, and just use physics for that to look natural". In flight the body fades into a flame (`player/flame.ts`), as in water into an orb: a small breathing core, and particles born there that rise by buoyancy, slow by drag, swirl with a little turbulence, keep a quarter of the flier's motion (so they stream behind in flight and lick upward in a hover), shrink and cool white-gold → gold → rose ember. Contained: dim particles, only the core blooms. Landing, the body forms again. (The mermaid kick above still drives the hidden skeleton.) Superseded by the geometric being (below).
- 2026-09-26: Samuel: "still the galaxies reflecting… better to not have any reflecting ray tracing at all but incredible skies when you look at them". No reflections anywhere: no `scene.environment` (nothing baked), no mirrored world in the lakes (the water reflects only the sky, from its own shader), no light-field pools on the ground (`groundLight` returns 0; the light field isn't rendered). Keep it so; put the effort into the sky itself.
- 2026-09-26: Samuel: "the underwater reflection is still annoying". The caustic web on the lake floors is gone, and the water's underside is a plain dim teal (no shimmering mirror); Snell's window (the sky straight up) stays.
- 2026-09-26: Samuel's `#stats` on his iPhone: 43 fps, worst 191 ms, "58227 draws" (that was `info.render.calls`, a running total; per frame it is ~220 `drawCalls`, now shown). On phones the top tier has no ambient occlusion (the costliest effect at native resolution for the least seen); the controller steps down below 50 fps (was 45) to chase 60; terrain streams one tile a frame (~10 ms each). Streaming the trees (~17 ms every 12 m) is the next hitch to spread out.
- 2026-09-26: Samuel: subtitles "still in" (his phone held an old copy: every subtitle path is off in the code; the `#stats` readout now shows the build time, `__BUILD__` from `vite.config.ts`); and "stop all narration… so that you just hear nature's sounds, like I was underwater enjoying the sound and the lady started talking". The player's card (⌄, now also with nothing playing) has "Only nature": every voice rests (the background narrator, the archetypes' greetings and teachings, the archive) until "Voices again"; it is the same switch as the menu's Narration and is saved. A choice you make yourself (tapping a vessel, asking a seated archetype) still speaks.
- 2026-09-26: Samuel sent four palettes as colour inspiration; four moods now take them (`moods.ts`, `H(hex, k)` scales each sRGB hex to the night's brightness): SE dawn the cyan-to-coral sky (#00B4D8, #9DB9CE, #E2C4CE, #F99B9B, #F1525E); W sunset the clouds catching orange (#737597, #785566, #D97D4A, #D15C26); SW ember the sunset beach (teal-sage above, gold sun, rust-lit dark clouds, near-black land; #AE3B13, #846146, #3B4840, #2F2E2C, #17191B); NW dusk the dusk palette (#25283A, #5E8590, #6B2F33, #D35A15, #545152). The sun's broad glow is kept low in these, so the cool upper colours survive.
- 2026-09-26: Samuel: "if you tap somewhere then the character sets course for that place"; the flying figure "a bit too basic… more like a geometric shape shifting sort of thing… something very alien geometric"; "if you tap and hold pressing on the heart of the character then the whole world becomes pitch black and then out of the character you start to see like lines connecting whole creation slowly, and then creation rebuilds itself… a 30 second sort of animation".
  - Tap to set course (touch too): walks there, runs if it's more than ~10–28 m away, flies there in the air; the stick takes over.
  - In flight the wanderer becomes a geometric being (`player/geoform.ts`, replaces the flame): a faceted glass core whose vertices rise and sink in travelling waves (stellated, spiked, rounded in turn), inside the turning edges of an octahedron (gold), a dodecahedron (pale blue) and a small tetrahedron at the heart, swelling in turn. Contained: thin edges, a dim core.
  - Genesis (`world/genesis.ts`): a still ~0.75 s press within ~70 px of the heart (`Input.onHold`), or H; not while flying, diving or seated. 0–3 s a veil over everything but a warm heart light; 3–16 s a web grows: from the heart to the few nearest things and the great ones (beings, planets, mountains), then from each thing to its neighbours, each reached along its shortest way (Dijkstra), so it spreads outward; each thing lights as a soft knot with a flare. Targets: trees and crystal clusters standing, lanterns, flowers within 45 m, beings, orbs, groves, homes, peaks. The camera draws back and up (`FollowCamera.lift`). 18–29 s the veil lifts into thick black air that thins from the heart outward (fog density and colour), then the sky, clouds and the moon's rays return; the fog-less lights come back at 24 s; the web fades. Bells at 264, 396, 528 Hz; the bed ducks. Lines are ribbons of constant pixel width (packed attributes: WebGPU allows 8 vertex buffers).
- 2026-09-26: Samuel's screenshots from his iPhone (flying high over the haze; flying low at sunset):
  - A hard straight split across the land when flying high. Two causes: the terrain streaming dropped finer tiles before the coarser ground beneath had risen again (a hole with straight edges while flying fast), and the far land's last tiles end ≥ 2048 m away while the haze covered it fully only at 2450 m. Now (`terrain.ts`): each level sinks only where the finer level's tiles stand and stay (`coveredBelow`); a tile left behind goes only after the ground beneath it is rebuilt (`retire`/`drop`); jobs are deduplicated and skip tiles passed by; about 3 ms of streaming a frame, 7 ms when much is waiting. The far land melts into the haze by 2020 m (`ijFog`).
  - The flier read as a white ball: its solids' edges were lines, one device pixel wide, invisible at 3x. They are now ribbons of constant pixel width (`geoform.ts` `edges`, widened in view space via `vertexNode`), dimmer; the core is coloured (blue to rose), not white. `gpuUniforms.px` (CSS pixels per metre at 1 m) is set each frame for such widths.
  - The readout's draws are taken right after each frame (`frameDraws`); his showed 136k, not a per-frame count.
- 2026-09-26: Samuel (screenshots): "what's that grainy split" / "hate that grainy split"; "new mode: autofly, character routes around going thru low fly touching nature and then up the galaxies"; of genesis's beams: "awful… it should be like thin geometric shapes bursting out, following the backend geometric shapes of the visuals"; a photo of a lake at golden hour: "like this colours… love the brown gold colours for at least one of the directions. But need the dusky blue and red cool constraint".
  - The grain: his readout had no build stamp, so he was on an old copy (the grain matches the ambient occlusion that phones no longer run; the current build renders clean at his spot). Updates (`core/fresh.ts`): the game asks the server for the current build (`cache: "no-store"`); if newer, before the journey it reloads at once (a new `?v=` address, so no cache answers), and on coming back to the game with no archive narration playing it saves and reloads; otherwise a whisper says so once.
  - Genesis redrawn (`world/genesis.ts`): no beams, no web. Thin geometric shapes burst from the heart one after another (circle, triangle, hexagon, octahedron, hexagram, seed of life, icosahedron, square, twelve-point circle, dodecahedron, octagon, tetrahedron), in gold, pale blue and rose, growing out and fading (hairline ribbons, `gpu/ribbons.ts`, shared with the flier). The world's own geometry lights up in a wave from the heart (to ~700 m by 17 s): every opaque mesh of the land, trees and stones, wilds, homes and the sky's vessels gets a wireframe twin (shared geometry, lifted 0.12 m along the normal so the lines don't break into dashes; near lines only, 140–320 m, as far meshes crowd into grain). Land gold, living things dim rose, vessels pale blue.
  - Autofly (`player/autofly.ts`; ⋮ → Autofly or P): it steers itself (turns at most 0.32 rad/s, wide arcs) from grove to home (low, 4.5 m over the highest ground of the next 40 m, 10 m/s, 60–95 s), then rises (22–30 s) to ~320 m and cruises among the planets and stars (17 m/s, 35–50 s), then falls back to low. The camera looks a little down low and up high. The stick, the button or a tap on the land takes over.
  - North-east is now golden hour over a lake (`moods.ts` GOLDEN, was haze): dusky blue sky (#46648C, #7F97B2), a peach-rose band low (#E5B7A0, red glow #C9786A), the low sun behind you laying brown-gold light on the land; the sky keeps its cool restraint.
- 2026-09-26: Samuel: the first geometric lines "I liked a lot better… the thinner lines and the dynamic movement… now it's thick-ish, I don't appreciate that"; "you still need to build the free fly option"; and the temple: "when the character goes into the temple… it's like a place to meditate and here is where we go through all the archetypes… no longer the open world… like another dimension, more solid… it's got to be the Egyptian temple… use as a basis… in 3D, the tarot cards… an interactive place for actual reflection and meditation. You can begin with the graphics and what it's like inside… I will probably have to refine the actual text and audio content."
  - Thin lines again: the flier's edges and genesis's shapes are hairline ribbons (0.45–0.5 CSS px, ~1.5 device px at 3x), at full brightness. Keep lines thin.
  - Free flight (⋮ → "Free flight: fly where you look", saved on the device): in the air the stick flies where the camera looks (forward climbs looking up, dives looking down); let go and you hover, no sinking; the view may look well up while flying (`FollowCamera.freeLook`). The button still rises; Land lands.
  - The Egyptian temple (`world/temple.ts`). This overrides the 2026-09-24 default "never temples": Samuel asked for one; deities and cult images stay out (the carvings are glyph-like forms: sun on the horizon, water, reeds, stars, lotus, spirals; the archetypes are the beings). A pylon gate (two battered towers, a glowing door) stands on dry ground ~185 m from the shore, clear of the homes; on the map as "The temple". Walk through its door: the screen fades, the open world is hidden and rests (not streamed or updated), and you are inside, a place apart built at x = 30000 (`TEMPLE_ORIGIN`; `heightAt` answers there from the temple's floor via `floorHook`). Hypostyle hall 24 × 62 m: two rows of papyrus-bundle columns painted in faded bands, walls carved in registers (canvas relief), a blue ceiling of gold stars, a raised central aisle with clerestory grilles and daylight behind them, slanting shafts, braziers (four real lights). Shrines: the Mind I–VII in niches down the left wall, the Body VIII–XIV down the right, each the being as on its card (the same `Beings`, one per pivot, 1.3×, turned to face the hall) on a plinth, its numeral and name on the lintel, a soft glow of its colour behind it. Through a gateway, the sanctuary: the Spirit XV–XXI in a ring facing the centre, the Choice (XXII) on a three-step round dais under an opening of light. Coming near, a being greets you, its name whispers, and (voices on, nothing speaking) its "Who are you?" answer plays. Walk out the door you came in by. Saving inside keeps the place outside the pylon. No genesis or autofly inside. Next (content with Samuel): meditation seats, reflections and practices per shrine.
- 2026-09-26: Samuel: "the center of the temple should be an interactive interface where you are able to go thru all the cards (now 3d rendered and reimagined); we will add interactive audios later"; "when entering the temple the character becomes covered with a robe, like an Ancient Greek character… head stays the same, character doesn't glow inside the temple".
  - The cards (`temple.ts` stage, `main.ts` cards UI): the Choice moved to a raised platform at the back of the sanctuary; at the centre, on the dais, an altar. Near it, "The cards" opens a panel (‹ ›, a strip of all 22 numerals grouped Mind / Body / Spirit / Choice, "Leave the cards"); a swipe or the arrow keys turn them. The view glides to face the altar. Each card is reimagined in 3D: a gilded double frame (2.7 × 4.3 m) on the altar, an opaque night-blue field with stars and a glow of the archetype's colour, the numeral above; within it the archetype's being with the objects it holds (its own `Beings`, made when first shown, centred by cancelling its landmark offset, 1.3×, no halo), turning slowly so its depth shows; changing card, the old one rises away and the new one rises into place, with a soft note. Subtitle: "The Matrix of the Mind" etc. The oculus shaft rests while the cards are open (it fell through the card). Audio per card comes later.
  - The robe (`player/robe.ts`): inside the temple the wanderer wears a chiton of pale linen, skinned to its own skeleton: over the body (its own form a little fuller; head, neck, forearms, hands, legs and feet left bare) and a long skirt in soft folds from the waist to the ankles, bound to the hips and toward the hem to the thighs so it swings with the steps; a belt and an ochre band at the hem. The body beneath is plain, matte and unlit (`plainSkin`); no halo, motes or ribbons. Outside, the light body returns. No flight inside the temple.
- 2026-09-26: Samuel: "the character is awful… I give up with the robe, just leave the character as is outside"; "the temple should feel more real life, more texture… Assassin's Creed Origins for inspo"; "I love the concept of the temple… I am crying… inside we don't want to be listening to waves. It has to be eerie, silent, and you only hear your steps hitting the rock… maybe a chanting, classic temple-ish chanting in the background".
  - The robe is gone (`player/robe.ts` removed; supersedes the robe above): in the temple the wanderer is the same light body as outside. Still no flight inside.
  - Sound inside (`core/audio.ts` `setTemple`): the water bed and the night tones fall silent (their own `worldDry`/`worldWet` buses); a long dark stone reverb (5.5 s, darkening as it fades); footsteps on stone (`stepStone`: the slap of a bare sole and a dull knock, the hall answering); a far chant: four low voices on D (D, E, F, G, A, d), sawtooth through two vowel formants (oo, oh, ah), each breathing its own long phrases, almost all of it the hall's echo. No background narrator and no archetype voices inside (only the name whispers); the archive player, if you start it, still plays.
  - Realism (`temple.ts` `stoneMaterial`, `floorMaterial`): every surface carries the rock scan's grain at two scales, its normals, and weathering (grime toward the floor, streaks, broad patches); the floor has sand drifted along the walls and in hollows, and a smoother, glossier worn aisle; the columns are rough stone under their faded bands; stronger sun through the clerestory, dimmer bounce; dust drifting in the air. The carvings (sunk relief, lip and cut, faded paint) were never stroked before; now drawn, finer and fainter, the star as five arms from a centre (not a pentagram), columns of small glyphs between ruled lines.
- 2026-09-26: Samuel asked to research better textures and graphics ("next level"). Findings: only four 1K texture sets (colour + normal; no roughness/AO), the ground projected from above (stretching on cliffs), visible repetition, texture fading to flat colour past ~150–520 m, painted-on carvings, code-built props. References: Journey's sand (rim, "ocean" specular, glitter, ripple normals), Sky (light and per-area palettes, layered cloud and fog, rim light), Assassin's Creed Origins (photogrammetry, four ground layers blended by height, broken pieces and dust, strong sun and bounce). Plan (Samuel: "Goooo"): 1) real material sets from Poly Haven (CC0) with full PBR, compressed; height blending, stochastic tiling, triplanar on slopes, detail to the horizon; 2) decals, real props, Journey sand; 3) baked light in the temple, volumetric shafts, LUT grading per mood, Sky-style clouds; pay for it with a slightly lower render scale + upscale/sharpen, measured on his phone.
  - Step 1a, the temple's stone (`temple.ts` `scan`, `stoneMaterial`): photo-scanned sandstone (Poly Haven, CC0, 1k, re-encoded with mozjpeg, 1.7 MB for the four sets; `public/textures/temple/`): walls `sandstone_blocks_08`, the pylon `sandstone_blocks_05`, columns/niches/ceiling `sandstone_cracks`, floor `red_sandstone_pavement`; colour, normal map, and the packed AO (deepens joints and cracks) and roughness. Laid in metres (a scan per 2.2–3.2 m). The carvings are now an overlay on the real stone (a data texture: mid grey no change, dark the cut, light the lip, colour the paint); the fake masonry lines are gone. The columns' bands tint the stone; the ceiling's blue and gold is paint worn through where the grain stands proud. A little more light (sun 3.8, bounce 0.8) and thinner air.
  - Step 1b, the ground (`terrain.ts` `groundMaterial`): no repeat shows (after Inigo Quilez: a slow noise picks one of eight offsets of each scan per stretch of ground, neighbours blend, both samples share the ground's derivatives via `.grad`, so no seams); cliffs and the mountains (steep ground, and all ground above ~22–60 m) are the `cliff_side` scan wrapped from the side (triplanar on x and z, its normals turned about each plane), near at 7 m a repeat and far at 63 m so its strata still read from a kilometre; snowfields keep their white with a trace of rock; the scans' detail reaches 300–1100 m (cliffs 1400–2400 m) instead of fading at 150–520 m; a broad two-scale variation over the land so far country is never one flat colour.
  - Steps 2 and 3 (Samuel: "Yes to all"):
    - Temple props (`temple.ts` `loadProps`, `buildRuin`): scanned Poly Haven models (`public/models/temple-props.glb`, 1.4 MB, built by `tools/build-temple-props.mjs`) drawn as instances: stone fire pits on stone pedestals (the braziers), clay and ceramic vessels gathered at the foot of the hall's shrines and along the sanctuary's walls, brass lamps before the Spirit's plinths and at the Choice's steps; fallen column drums and blocks in the corners, in the same scanned stone.
    - Journey's sand (`terrain.ts`): ripples combed by the wind (two wavelengths, bent by noise, faint), a soft broad sheen off them toward the moon or the low sun (`fogUniforms.glowDir`/`glow`, faint and near only: never a glare sliding over the ground), a pale rim at grazing angles; the glitter reaches 30 m.
    - Light beams in the temple: soft volumes (open tubes, brightest where you look through their depth, dust turning in them, melting near the lens) instead of crossed planes, each laying a warm pool where it meets the floor.
    - Colour grading (`post.ts`, `gradeUniforms` in `tsl.ts`): after tone mapping, per mood (`moods.ts` `GRADES`, blended like the moods): colour lifted into the shadows, a tint on the highlights, saturation and contrast (golden hour: dusky blue shadows, gold light; ember: teal shadows, gold; dusk: reds and blues); the temple its own warm grade.
    - Clouds in three layers (`atmosphere.ts`): billows low, smaller ones above, and high thin sheets combed into streaks; all lit from the glow's direction (the low sun at dawn and dusk, the moon by night), with the silver lining when it's behind them.
    - Not done: a lower render scale with upscaling (Samuel asked for maximum resolution; his phone holds 60 fps at native 3x), KTX2 textures (next if memory or loading becomes a problem).
